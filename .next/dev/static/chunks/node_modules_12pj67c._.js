...
function createRenderSearchParams(underlyingSearchParams, workStore, requestStore, isRuntimePrefetchable) {
    ...
    if (asyncApiPromises) {
        ...
        return createStagedRenderSearchParams(workStore, asyncApiPromises, isRuntimePrefetchable, underlyingSearchParams, userspaceSearchParams);
    }
...
function createStagedRenderSearchParams(workStore, asyncApiPromises, isRuntimePrefetchable, underlyingSearchParams, userspaceSearchParams) {
    const trigger = isRuntimePrefetchable ? asyncApiPromises.earlySharedSearchParamsParent : asyncApiPromises.sharedSearchParamsParent;
    if ("TURBOPACK compile-time truthy", 1) {
...
function createServerParamsForServerSegment(underlyingParams, optionalCatchAllParamName, varyParamsAccumulator, isRuntimePrefetchable) {
    ...
    if (workUnitStore) {
        switch(workUnitStore.type){
            ...
            case 'request':
                {
                    return createRenderParamsForPage(workStore, workUnitStore, underlyingParams, optionalCatchAllParamName, varyParamsAccumulator, isRuntimePrefetchable);
                }
            default:
                workUnitStore;
        }
...
function createRenderParamsForPage(workStore, workUnitStore, underlyingParams, optionalCatchAllParamName, varyParamsAccumulator, isRuntimePrefetchable) {
    ...
    if (varyParamsAccumulator) {
        userspaceParams = (0, _varyparams.createVaryingParams)(varyParamsAccumulator, userspaceParams, optionalCatchAllParamName);
    }
    if (stagedRendering && asyncApiPromises) {
        return createStagedRenderParams(workStore, workUnitStore, stagedRendering, asyncApiPromises, underlyingParams, userspaceParams, isRuntimePrefetchable);
    }
    // No staged rendering = no cacheComponents, or cacheComponents prod without cachedNavigations
    if ("TURBOPACK compile-time truthy", 1) {
        const fallbackParams = workUnitStore.fallbackParams;
...
function createStagedRenderParams(workStore, workUnitStore, stagedRendering, asyncApiPromises, underlyingParams, userspaceParams, isRuntimePrefetchable) {
    const promise = createStagedRenderParamsImpl(workUnitStore, stagedRendering, asyncApiPromises, underlyingParams, userspaceParams, isRuntimePrefetchable);
    if ("TURBOPACK compile-time truthy", 1) {
        return instrumentParamsPromiseWithDevWarnings(underlyingParams, promise, workStore);
    } else //TURBOPACK unreachable
    ;
}
function createStagedRenderParamsImpl(workUnitStore, stagedRendering, asyncApiPromises, /** The actual param values, without any instrumentation */ underlyingParams, /** The params object to return to userspace, possibly wrapped in a proxy */ userspaceParams, isRuntimePrefetchable) {
    const hasFallbackParams = hasFallbackRouteParams(underlyingParams, workUnitStore.fallbackParams);
    ...
    if (("TURBOPACK compile-time value", false) && // Params are non-empty, and there's no fallback params, so all params are static
    !isEmptyParams(underlyingParams) && !hasFallbackParams) //TURBOPACK unreachable
    ;
    // Otherwise, only delay if we have fallback params
    if (hasFallbackParams) {
        return createParamsPromiseFromTrigger(isRuntimePrefetchable ? asyncApiPromises.earlySharedParamsParent : asyncApiPromises.sharedParamsParent, userspaceParams);
    }
...
function makeDynamicallyTrackedParamsWithDevWarnings(underlyingParams, userspaceParams, hasFallbackParams, workStore, requestStore) {
    ...
    // supports copying with spread and we don't want to unnecessarily
    // instrument the promise with spreadable properties of ReactPromise.
    const promise = hasFallbackParams ? (0, _dynamicrenderingutils.makeDevtoolsIOAwarePromise)(userspaceParams, requestStore, _stagedrendering.RenderStage.Runtime) : Promise.resolve(userspaceParams);
...
//# sourceMappingURL=node_modules_12pj67c._.js.map